...
def load_models(model_dir="./models"):
    ...
    """
    global _short_model, _long_model, _short_scaler, _long_scaler
    
    try:
        model_files = {
            '_short_model': 'short_model.pkl',
            '_long_model': 'long_model.pkl', 
            '_short_scaler': 'short_scaler.pkl',
            '_long_scaler': 'long_scaler.pkl'
        }
        
        for attr_name, filename in model_files.items():
            filepath = os.path.join(model_dir, filename)
            if os.path.exists(filepath):
                globals()[attr_name] = joblib.load(filepath)
...
def validate_input_csv(df):
    ...
    # 데이터 길이 검사
    if len(df) < _min_data_length:
        return False, f"데이터가 충분하지 않습니다. 최소 {_min_data_length}개 행이 필요합니다."
...
# 지연 피처를 생성하는 함수
def create_lagged_features(df, feature_name, lag, time_minutes):
    """
    지연 피처를 생성합니다.
...
# 변화율 피처를 생성하는 함수
def create_roc_features(df, feature_name, roc_period, time_minutes, agg_type):
    """
    변화율 피처를 생성합니다.
...
# 피처 이름에서 변형 정보를 추출하는 함수
def extract_feature_name_info(feature_name):
    """
...
def prepare_features(df, model_type):
    ...
    for feature_name in feature_names:
        feature_info = extract_feature_name_info(feature_name)
        ...
        if feature_info['type'] == 'original':
            ...
            prepared_features[feature_name] = create_lagged_features(
                df, feature_info['base_feature'], 
                feature_info['lag'], feature_info['time_minutes']
            )
        
        elif feature_info['type'] == 'roc':
            prepared_features[feature_name] = create_roc_features(
                df, feature_info['base_feature'],
            ...
            print(f"알 수 없는 피처 타입: {feature_name}")
...
# 최종 예측값을 계산하는 함수
def final_pred(short_model_pred, long_model_pred):
    """
    최종 예측값을 계산합니다.
...
# 예측값에 따른 상태를 반환하는 함수
def get_status(final_prediction):
    """
    예측값에 따른 상태를 반환합니다.
...
# 메인 추론 함수
def predict(csv_path, target_time=None, model_dir="./models"):
    """
    ...
    Returns:
        결과 메시지
    """
    global _short_model, _long_model
    
    try:
        # 모델이 로드되지 않았다면 로드
        if _short_model is None or _long_model is None:
        ...
        # 입력 데이터 검증
        is_valid, error_msg = validate_input_csv(df)
        if not is_valid:
            return f"입력 데이터 검증 실패: {error_msg}"
        
        # 피처 검증
        is_valid, error_msg = validate_features(df)
        if not is_valid:
        ...
        # 피처 준비
        try:
            short_features = prepare_features(df, 'short')
            long_features = prepare_features(df, 'long')
            print("피처 준비 완료")
        except Exception as e:
        ...
        # 예측 수행
        try:
            short_pred = _short_model.predict(short_features.iloc[-1:])[0]
            long_pred = _long_model.predict(long_features.iloc[-1:])[0]
            print(f"예측 완료 - 단기: {short_pred:.2f}, 장기: {long_pred:.2f}")
        except Exception as e:
            return f"예측 오류: {str(e)}"
        
        # 최종 예측값 계산
        final_prediction = final_pred(short_pred, long_pred)
        status = get_status(final_prediction)
...
# 편의 함수
def run_inference(csv_path, target_time=None, model_dir="./models"):
    """
    추론 시스템을 실행하는 편의 함수
    ...
    """
    try:
        result = predict(csv_path, target_time, model_dir)
        return result
    except Exception as e:
