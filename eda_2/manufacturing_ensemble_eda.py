...
class ManufacturingEnsembleEDA:
    def __init__(self, X_a, X_b, y, long_term_model, short_term_model, scaler_a, scaler_b, basic_tags=None, threshold=70000):
        """
        제조 산업 센서 데이터 ensemble 모델 EDA 클래스
        
        Parameters:
        X_a: Long Term Model의 features (tags + lag features)
        X_b: Short Term Model의 features (tags + lag features)
        y: target
        long_term_model: Long Term gradient boosting 모델
        short_term_model: Short Term gradient boosting 모델
        scaler_a: Long Term Model용 스케일러
        scaler_b: Short Term Model용 스케일러
        basic_tags: 기본 태그 리스트 (예: ['Tag_01', 'Tag_02', ...])
        threshold: Long Term 모델 선택 기준 임계값 (기본값: 70000)
        """
        self.X_a = X_a
        self.X_b = X_b
        self.y = y
    ...
    def ensemble_predict(self, X_a, X_b):
        """Ensemble 예측 수행"""
        # 스케일링 적용
        scaled_X_a = pd.DataFrame(index=X_a.index, columns=X_a.columns, data=self.scaler_a.transform(X_a))
        scaled_X_b = pd.DataFrame(index=X_b.index, columns=X_b.columns, data=self.scaler_b.transform(X_b))
        
        # 모델 예측
        pred_a = self.long_term_model.predict(scaled_X_a).reshape(-1,1)*1000
        pred_b = self.short_term_model.predict(scaled_X_b).reshape(-1,1)*1000
        
        # Long Term 모델의 예측값이 threshold를 넘으면 Long Term 사용, 아니면 Short Term 사용
        ensemble_pred = np.where(pred_a > self.threshold, pred_a, pred_b)
        
        return ensemble_pred, pred_a, pred_b
    
    def threshold_analysis(self):
        """Threshold 초과 감지 능력 분석"""
        print("=== Threshold 초과 감지 능력 분석 ===")
        ...
        # 정확도 계산
        correct_detection = np.sum((threshold_exceeded == actual_exceeded))
        total_samples = len(self.y)
        accuracy = correct_detection / total_samples
        
        # 상세 분석
        true_positive = np.sum((threshold_exceeded == True) & (actual_exceeded == True))
        false_positive = np.sum((threshold_exceeded == True) & (actual_exceeded == False))
        true_negative = np.sum((threshold_exceeded == False) & (actual_exceeded == False))
        false_negative = np.sum((threshold_exceeded == False) & (actual_exceeded == True))
        ...
        return {
            ...
            'long_term_selected': long_term_selected,
            'short_term_selected': short_term_selected
        }
    ...
    def analyze_basic_tag_transformations(self):
        ...
        if common_basic_tags:
            ...
            for tag in common_tags:
                ...
                if hasattr(self.long_term_model, 'feature_importances_'):
                    a_feat_importance = [self.long_term_model.feature_importances_[self.tag_names_a.index(feat)] 
                ...
                if hasattr(self.short_term_model, 'feature_importances_'):
                    b_feat_importance = [self.short_term_model.feature_importances_[self.tag_names_b.index(feat)] 
    ...
    def feature_importance_analysis(self):
        ...
        if hasattr(self.long_term_model, 'feature_importances_'):
            importance_a = self.long_term_model.feature_importances_
        else:
            importance_a = np.abs(self.long_term_model.coef_) if hasattr(self.long_term_model, 'coef_') else np.ones(len(self.tag_names_a))
        
        # 모델 B의 feature importance
        if hasattr(self.short_term_model, 'feature_importances_'):
            importance_b = self.short_term_model.feature_importances_
        else:
            importance_b = np.abs(self.short_term_model.coef_) if hasattr(self.short_term_model, 'coef_') else np.ones(len(self.tag_names_b))
    ...
    def ensemble_decision_analysis(self):
        ...
        # 각 모델의 예측
        pred_a = self.long_term_model.predict(self.X_a).reshape(-1,1)*1000
        pred_b = self.short_term_model.predict(self.X_b).reshape(-1,1)*1000
        
        # Long Term 모델의 예측값이 threshold를 넘으면 Long Term 사용, 아니면 Short Term 사용
        use_long_term = pred_a > self.threshold
        
        plt.figure(figsize=(15, 10))
        
        # 결정 분포
        plt.subplot(2, 3, 1)
        decision_counts = [np.sum(use_long_term), np.sum(~use_long_term)]
        plt.pie(decision_counts, labels=['Long Term Model', 'Short Term Model'], autopct='%1.1f%%',
        ...
        plt.subplot(2, 3, 2)
        # Confidence 제거
        plt.hist(pred_a, bins=50, alpha=0.7, color='lightblue', edgecolor='black')
        plt.axvline(x=self.threshold, color='red', linestyle='--', linewidth=2, label=f'Threshold: {self.threshold}')
        plt.xlabel('Model A Prediction')
        plt.ylabel('Frequency')
        plt.title('Model A Prediction Distribution')
        plt.legend()
        ...
        plt.scatter(pred_a[use_long_term], error_a[use_long_term], 
                   alpha=0.6, label='Long Term Model Used', color='blue')
        plt.scatter(pred_a[~use_long_term], error_b[~use_long_term], 
                   alpha=0.6, label='Short Term Model Used', color='red')
        ...
        # 모델별 예측값 분포
        plt.subplot(2, 3, 4)
        plt.hist(pred_a, alpha=0.7, label='Long Term Model', bins=30, color='skyblue')
        plt.hist(pred_b, alpha=0.7, label='Short Term Model', bins=30, color='lightcoral')
        plt.xlabel('Predicted Values')
        plt.ylabel('Frequency')
        ...
        plt.axvline(x=0, color='red', linestyle='--', alpha=0.7)
        plt.xlabel('Prediction Difference (Long Term - Short Term)')
        plt.ylabel('Frequency')
        ...
        if len(self.X_a) > 100:
            ...
            for i in range(0, len(self.X_a) - window_size, window_size):
                window_decisions = use_long_term[i:i+window_size]
            ...
            plt.ylabel('Long Term Usage Rate')
            plt.title('Long Term Usage Over Time')
            plt.grid(True, alpha=0.3)
        
        plt.tight_layout()
        plt.show()
        
        print(f"Long Term Model 사용률: {np.mean(use_long_term):.2%}")
        print(f"Short Term Model 사용률: {np.mean(~use_long_term):.2%}")
        # Confidence 제거
        print(f"평균 예측값: {np.mean(pred_a):.2f}")
    ...
    def comprehensive_analysis(self, generate_html_report=True):
        ...
        print("\n=== 종합 결론 ===")
        print("1. 주요 발견사항:")
        print(f"   - Long Term Model 최고 상관관계 태그: {target_corr_a.index[0]} (상관계수: {target_corr_a.iloc[0]:.3f})")
        print(f"   - Short Term Model 최고 상관관계 태그: {target_corr_b.index[0]} (상관계수: {target_corr_b.iloc[0]:.3f})")
        print(f"   - Long Term Model 사용률: {np.mean(use_long_term):.2%}")
        print(f"   - Short Term Model 사용률: {np.mean(~use_long_term):.2%}")
        ...
        if generate_html_report:
            try:
                ...
                html_filename = generate_eda_html_report(results)
                print(f"\nHTML 리포트가 생성되었습니다: {html_filename}")
            except ImportError:
                print("HTML 리포트 생성 기능을 사용할 수 없습니다. html_report_utils.py 파일을 확인해주세요.")
        
        return results

# 사용 예시 함수
def run_manufacturing_eda(X_a, X_b, y, long_term_model, short_term_model, scaler_a, scaler_b, basic_tags=None, threshold=70000, generate_html_report=True):
    """
    ...
    basic_tags: 기본 태그 리스트 (예: ['Tag_01', 'Tag_02', ...])
    threshold: Long Term 모델 선택 기준 임계값 (기본값: 70000)
    generate_html_report: HTML 리포트 생성 여부 (기본값: True)
    
    Returns:
    dict: 분석 결과 딕셔너리
    """
    eda = ManufacturingEnsembleEDA(X_a, X_b, y, long_term_model, short_term_model, scaler_a, scaler_b, basic_tags, threshold)
    results = eda.comprehensive_analysis(generate_html_report=generate_html_report)
    return results
...
def test_eda():
    ...
    # EDA 실행
    results = run_manufacturing_eda(X_a, X_b, y, long_term_model, short_term_model, scaler_a, scaler_b, basic_tags, threshold=70000)
