...
def validate_input_csv(df: pd.DataFrame) -> Tuple[bool, str]:
    """
    입력 CSV의 유효성을 검사합니다.
...
def get_required_features() -> Dict[str, List[str]]:
    """
...
def create_lagged_features(df: pd.DataFrame, feature_name: str, 
...
def create_roc_features(df: pd.DataFrame, feature_name: str,
...
def extract_feature_name_info(feature_name: str) -> Dict:
    """
...
def final_pred(short_model_pred: float, long_model_pred: float) -> float:
    """
    최종 예측값을 계산합니다.
...
def get_status(final_prediction: float) -> str:
    """
    예측값에 따른 상태를 반환합니다.
...
def predict(csv_path: str, target_time: Optional[Union[str, datetime]] = None, 
