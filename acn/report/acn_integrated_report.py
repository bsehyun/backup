...
class ACNIntegratedReportGenerator:
    ...
    def integrate_all_results(self, 
                            ...
                            efficiency_optimization_results=None):
        """
        모든 분석 결과를 통합
        ...
        efficiency_optimization_results: main_efficiency_optimization 결과
        """
        print("=" * 80)
        ...
        self.report_data = {
            ...
            'efficiency_optimization': efficiency_optimization_results
        }
    ...
    def _generate_analysis_summary(self):
        ...
        # 7. 효율성 최적화 인사이트 수집
        self._collect_efficiency_insights(summary)
    ...
    def _collect_efficiency_insights(self, summary):
        """효율성 최적화 인사이트 수집"""
        insights = {}
        
        # 효율성 최적화 결과
        if self.report_data['efficiency_optimization']:
            eff_data = self.report_data['efficiency_optimization']
            if 'report' in eff_data:
                report = eff_data['report']
                if 'key_findings' in report:
                    insights['efficiency_findings'] = report['key_findings']
                if 'recommendations' in report:
                    insights['efficiency_recommendations'] = report['recommendations']
                if 'input_guidance' in report:
                    insights['input_guidance'] = report['input_guidance']
            
            if 'model_results' in eff_data:
                model_results = eff_data['model_results']
                best_model_name = eff_data.get('best_model_name', 'Unknown')
                if best_model_name in model_results:
                    best_model_data = model_results[best_model_name]
                    insights['efficiency_model'] = {
                        'name': best_model_name,
                        'test_r2': best_model_data['test_r2'],
                        'cv_mean': best_model_data['cv_mean']
                    }
        
        summary['efficiency_insights'] = insights
        ...
        <body>
            <div class="container">
                ...
                <nav class="nav">
                    <ul>
                        ...
                        <li><a href="#efficiency">📈 효율성 최적화</a></li>
                        <li><a href="#user_report">📝 사용자 리포트</a></li>
                ...
                <section id="overview" class="section">
                    ...
                    <div class="analysis-grid">
                        ...
                        <div class="analysis-card">
                            <h4>효율성 최적화</h4>
                            <p><span class="status-indicator {'status-completed' if self.report_data['efficiency_optimization'] else 'status-missing'}"></span>
                            {'완료' if self.report_data['efficiency_optimization'] else '미실행'}</p>
                            <p>Input 증가 시 효율성 유지 방안</p>
                        </div>
                    </div>
                </section>
                ...
                <section id="efficiency" class="section">
                    <h2>📈 효율성 최적화</h2>
                    {self._generate_efficiency_html()}
                </section>
                
                <section id="user_report" class="section">
                    <h2>📝 사용자 리포트</h2>
                    <div class="user-report-section">
                        <h3>사용자 정의 분석 리포트</h3>
                        <div class="text-report-area">
                            <textarea id="userTextReport" placeholder="여기에 사용자의 분석 결과, 해석, 또는 추가 인사이트를 입력하세요..." 
                                      style="width: 100%; height: 300px; padding: 15px; border: 2px solid #ddd; border-radius: 5px; font-family: 'Malgun Gothic', Arial, sans-serif; font-size: 14px; line-height: 1.6; resize: vertical;"></textarea>
                            <div style="margin-top: 10px;">
                                <button onclick="saveUserReport()" style="background-color: #3498db; color: white; padding: 10px 20px; border: none; border-radius: 5px; cursor: pointer; font-size: 14px;">리포트 저장</button>
                                <button onclick="clearUserReport()" style="background-color: #95a5a6; color: white; padding: 10px 20px; border: none; border-radius: 5px; cursor: pointer; font-size: 14px; margin-left: 10px;">내용 지우기</button>
                            </div>
                        </div>
                        <div id="savedReport" style="margin-top: 20px; padding: 15px; background-color: #f8f9fa; border-radius: 5px; border-left: 4px solid #3498db; display: none;">
                            <h4>저장된 사용자 리포트:</h4>
                            <div id="savedReportContent"></div>
                        </div>
                    </div>
                </section>
            ...
            <script>
                function saveUserReport() {{
                    const textarea = document.getElementById('userTextReport');
                    const savedReport = document.getElementById('savedReport');
                    const savedReportContent = document.getElementById('savedReportContent');
                    ...
                    savedReportContent.innerHTML = '<p style="white-space: pre-wrap; line-height: 1.6;">' + textarea.value + '</p>';
                    savedReport.style.display = 'block';
                    
                    // 로컬 스토리지에 저장
                    localStorage.setItem('userReport', textarea.value);
                    
                    alert('사용자 리포트가 저장되었습니다.');
                }}
                
                function clearUserReport() {{
                    if (confirm('리포트 내용을 지우시겠습니까?')) {{
                        document.getElementById('userTextReport').value = '';
                ...
                window.onload = function() {{
                    ...
                    if (savedReport) {{
                        document.getElementById('userTextReport').value = savedReport;
                        document.getElementById('savedReportContent').innerHTML = '<p style="white-space: pre-wrap; line-height: 1.6;">' + savedReport + '</p>';
    ...
    def _generate_feature_importance_html(self):
        ...
        if 'sobol_top' in importance:
            ...
            for feature in importance['sobol_top']:
                html += f"<tr><td>{feature['feature']}</td><td>{feature['s1']:.4f}</td><td>{feature['S1_conf']:.4f}</td></tr>"
            html += "</table>"
    ...
    def _generate_efficiency_html(self):
        """효율성 최적화 HTML 생성"""
        efficiency_insights = self.analysis_summary.get('efficiency_insights', {})
        if not efficiency_insights:
            return "<p>효율성 최적화 데이터가 없습니다.</p>"
        
        html = ""
        
        # 효율성 모델 성능
        if 'efficiency_model' in efficiency_insights:
            model = efficiency_insights['efficiency_model']
            html += f"""
            <div class="insight-box">
                <h4>효율성 예측 모델</h4>
                <p><strong>최고 성능 모델:</strong> {model['name']}</p>
                <p><strong>Test R² Score:</strong> <span class="metric">{model['test_r2']:.4f}</span></p>
                <p><strong>CV Score:</strong> {model['cv_mean']:.4f}</p>
            </div>
            """
        
        # 주요 발견사항
        if 'efficiency_findings' in efficiency_insights:
            html += "<h4>주요 발견사항</h4>"
            for finding in efficiency_insights['efficiency_findings']:
                html += f'<div class="finding-box"><p>{finding}</p></div>'
        
        # 권장사항
        if 'efficiency_recommendations' in efficiency_insights:
            html += "<h4>권장사항</h4>"
            for recommendation in efficiency_insights['efficiency_recommendations']:
                html += f'<div class="recommendation-box"><p>{recommendation}</p></div>'
        
        # Input 증가 가이드
        if 'input_guidance' in efficiency_insights:
            guidance = efficiency_insights['input_guidance']
            html += "<h4>Input 증가 시 Control 가이드</h4>"
            html += "<table><tr><th>시나리오</th><th>새로운 Input</th><th>최적 Control 값</th></tr>"
            
            for scenario, data in guidance.items():
                scenario_name = scenario.replace('increase_', 'Input 증가 ').replace('%', '%')
                new_input = data.get('new_input', 'N/A')
                optimal_controls = data.get('optimal_controls', {})
                
                controls_text = ""
                for control_name, control_value in optimal_controls.items():
                    controls_text += f"{control_name}: {control_value:.4f}<br>"
                
                html += f"<tr><td>{scenario_name}</td><td>{new_input:.2f}</td><td>{controls_text}</td></tr>"
...
def create_integrated_report(advanced_exp_results=None,
    ...
    generator.integrate_all_results(
        ...
        efficiency_optimization_results=efficiency_optimization_results
    )
