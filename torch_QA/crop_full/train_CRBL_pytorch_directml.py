...
from model_pytorch_dual_directml import (
    create_crop_model, create_full_model, create_model, 
    count_parameters, print_model_info, get_device
)
...
model_save_name = f"CRBL_250328_pytorch_directml_{MODEL_TYPE}"

print(f"Training {MODEL_TYPE} model with DirectML...")
print(f"Model will be saved as: {model_save_name}.pth")
...
def main():
    ...
    if MODEL_TYPE == "crop":
        model = create_crop_model(input_shape=input_shape, num_classes=num_classes)
        print_model_info(model, "Crop")
    elif MODEL_TYPE == "full":
        model = create_full_model(input_shape=input_shape, num_classes=num_classes)
        print_model_info(model, "Full")
    else:
